...
namespace ChessChecker
{
    public class Pawn : Piece
    {
        ...
        public override List<Square> PossibleMoves(Square[,] board, (int, int) piecePosition)
        {
            ...
            if (leftMove.Item1 >= 0 && leftMove.Item2 >=0 && leftMove.Item1 <= 7 && leftMove.Item2 <=7) // Make sure move is on table
            {
                moves.Add(board[leftMove.Item1, leftMove.Item2]);
            }
            if (rightMove.Item1 >= 0 && rightMove.Item2 >= 0 && rightMove.Item1 <= 7 && rightMove.Item2 <= 7) // Make sure move is on table
            {
